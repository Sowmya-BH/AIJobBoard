...
def ats_node(state):
    ...
    ats = rq_tools.call("score_ats", resume_text=resume_text, jd_text=jd_text)
    hr = rq_tools.call("score_hr", resume_text=resume_text, jd_text=jd_text)
    expl = rq_tools.call("explain_score", resume_text=resume_text, jd_text=jd_text)
...
#                                  "\n\nADDITIONAL EXPERIENCE:\n" + info)
#         update["user_edit"] = info
#         update["how_to_add"] = _how_to_add(info, state.get("ats", {}))
#     elif action == "upload_resume":
#         update["resume_text"] = payload.get("resume_text", state.get("resume_text", ""))
#     elif action == "ask":
#         update["user_question"] = payload.get("text", "")
...
# def _how_to_add(info: str, ats: dict) -> str:
#     missing = ats.get("_raw", {}).get("missing_keywords", [])
#     covered = [k for k in missing if k.lower() in info.lower()]
#     if llm.available():
#         return llm.generate(
...
#                             job_title=job.get("title", ""))
#         letter = res.get("full_text")
#         if not letter:   # ResumeHQ letters need cloud/Pro; draft with Gemini instead
#             letter = llm.generate(
#                 f"Write a concise, specific cover letter for {job['title']} at "
#                 f"{job['company']}. Ground it ONLY in this resume:\n{resume_text[:4000]}\n"
#                 f"Job:\n{jd_text}\nDo not invent experience.")
...
#     elif action == "tailored_resume":
#         missing = state.get("ats", {}).get("_raw", {}).get("missing_keywords", [])
#         artifacts["tailored_resume"] = llm.generate(
#             f"Rewrite a resume summary + 5 bullets for {job['title']} at "
#             f"{job['company']}. Base ONLY on this resume:\n{resume_text[:4000]}\n"
#             f"Incorporate any of these missing keywords the candidate can honestly "
#             f"claim: {missing}. Do not invent experience.")
...
# #     hr = rq_tools.call("score_hr", resume_text=resume_text, jd_text=jd_text)
# #     expl = rq_tools.call("explain_score", resume_text=resume_text, jd_text=jd_text)
# #     return {"ats": _reshape(job, jd_text, ats, hr, expl), "jd_text": jd_text}


# # def _reshape(job, jd_text, ats, hr, expl):
# #     """ResumeHQ raw dicts -> matched / missing / advice. Grounded: on tool error
# #     we surface it and DO NOT fabricate."""
# #     err = ats.get("error") or hr.get("error")
# #     matched_kw = ats.get("matched_keywords", []) or []
# #     missing_kw = ats.get("missing_keywords", []) or []

# #     matched = [f"Both resume and job include {k}." for k in matched_kw[:8]]
# #     missing = [f"The job requires {k}, absent from the resume." for k in missing_kw[:8]]
# #     for c in (hr.get("concerns") or [])[:3]:
# #         missing.append(c)

# #     advice = []
# #     explanation = expl.get("explanation") if isinstance(expl, dict) else None
# #     if isinstance(explanation, dict):
# #         top = explanation.get("top_missing_keywords", [])
# #         if top:
# #             advice.append(f"Add these missing keywords where they truly apply: {', '.join(top[:6])}.")
# #         for key in ("quick_wins", "suggestions", "section_tips"):
# #             v = explanation.get(key)
# #             if isinstance(v, list):
# #                 advice.extend(str(x) for x in v[:3])
...
# #         "suggested_questions": hr.get("suggested_questions", []),
# #         "error": err,
# #         "_raw": {"matched_keywords": matched_kw, "missing_keywords": missing_kw},
# #     }
