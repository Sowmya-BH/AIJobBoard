...
GRAPH = build_graph()
userstore.init()  # create MongoDB indexes
...
def chat(req: ChatReq, user=Depends(auth.current_user)):
    ...
    creds = userstore.get_api_credentials(user["id"])
    if creds and creds.get("encrypted_api_key"):
    ...
    # No BYO key -> use the server's Gemini (the same path that works for parsing)
    from agent import llm
    if llm.available():
        return {"reply": llm.generate(msg, system=system)}
    raise HTTPException(400, "No API key set. Add one in Settings, or set "
...
@app.post("/api/action")
def action(req: ActionReq, user=Depends(auth.current_user)):
    ...
    # Re-inject the user's BYO key into graph STATE every action, so keys added
    # mid-session are used by extras (cover letter / interview Qs / tailored /
    # ask). resume= only carries the interrupt payload — it does NOT update
    # state — so we write llm_creds via update= on the same thread first.
    creds = userstore.get_api_credentials(user["id"]) or {}
    if creds.get("encrypted_api_key"):
        key = crypto.decrypt(creds["encrypted_api_key"])
        if key:
            _lc = {"provider": creds.get("provider"), "model": creds.get("model"),
                   "api_key": key, "base_url": creds.get("base_url", "")}
            # (a) update the checkpointed state, and (b) carry the creds ON the
            # resume payload so human_review_node writes them into state right
            # before extras runs — robust to mid-session saves regardless of
            # checkpointer/interrupt timing.
            GRAPH.update_state(_cfg(req.thread_id), {"llm_creds": _lc})
            cmd["llm_creds"] = _lc
...
# export RQ_SCORER_KIND=gradio
# export RQ_SCORER_URL=https://<username>-resumehq-scorer.hf.space   # or <username>/resumehq-scorer
# # optional, only for a PRIVATE Space:
# export RQ_SCORER_HF_TOKEN=hf_xxx
