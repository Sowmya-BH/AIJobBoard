...
RAW_SCORER_URL = (os.environ.get("RQ_SCORER_URL", "") or "").strip()  # unmodified (for gradio_client / "user/space" ids)
SCORER_TIMEOUT = float(os.environ.get("RQ_SCORER_TIMEOUT", "60"))
# "rest"  -> POST to a FastAPI sidecar (/score/ats ...)
# "gradio"-> call a Gradio Space via gradio_client (api_name=/score_ats ...)
SCORER_KIND = (os.environ.get("RQ_SCORER_KIND", "rest") or "rest").strip().lower()
REMOTE_ENDPOINTS = {
    "score_ats": "/score/ats",
    "score_hr": "/score/hr",
...
# Gradio Space api_names (see the Space's app.py: b.click(..., api_name=...)).
GRADIO_ENDPOINTS = {
    "score_ats": "/score_ats",
    "score_hr": "/score_hr",
    "explain_score": "/explain",
...
def call(tool: str, **kwargs) -> dict:
    ...
    # (remote, gradio) a ResumeHQ Gradio Space (ZeroGPU) via gradio_client
    if SCORER_URL and SCORER_KIND == "gradio" and tool in GRADIO_ENDPOINTS:
        t0 = time.time()
        result = _call_gradio(GRADIO_ENDPOINTS[tool],
                              {k: kwargs[k] for k in spec["args"]})
        _record({"id": call_id, "ts": ts, "tool": tool, "via": "gradio",
    ...
    if SCORER_URL and SCORER_KIND != "gradio" and tool in REMOTE_ENDPOINTS:
        t0 = time.time()
...
def _get_gradio_client():
    """Create (once) a gradio_client for the Space. Accepts a full URL
    (https://<user>-<space>.hf.space) or a repo id (<user>/<space>).
    The token kwarg name differs across gradio_client versions (hf_token vs
    token), so try both and fall back to anonymous."""
    global _gradio_client
    if _gradio_client is None:
        from gradio_client import Client   # lightweight, torch-free
        src = RAW_SCORER_URL or SCORER_URL
        token = os.environ.get("RQ_SCORER_HF_TOKEN") or None
        if token:
            for kw in ("hf_token", "token"):
                try:
                    _gradio_client = Client(src, **{kw: token})
                    break
                except TypeError:
                    continue
        if _gradio_client is None:
            _gradio_client = Client(src)   # anonymous (public Space)
    return _gradio_client


def _call_gradio(api_name: str, payload: dict) -> dict:
    """Call a Gradio Space endpoint; normalize output to a dict. Any failure
    (asleep Space, network) returns a clean error dict, never raises."""
    try:
        c = _get_gradio_client()
        res = c.predict(payload.get("resume_text", ""),
                        payload.get("jd_text", ""), api_name=api_name)
    except Exception as e:
        return {"error": "scorer_unreachable",
                "message": f"{type(e).__name__}: {e} (gradio {RAW_SCORER_URL})"}
    if isinstance(res, dict):
        return res
    if isinstance(res, str):          # gr.JSON may arrive as a temp path or json string
        try:
            ...
            return {"error": "bad_return", "message": "non-JSON gradio result"}
    return {"error": "bad_return", "message": f"unexpected type {type(res).__name__}"}
